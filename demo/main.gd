...
const DLC_FILENAME := "sku-00-dlc-message.txt"
const DLC_PRODUCT_SKU := "00"
...
var dlc_processing := false
var dlc_displayed := false
...
@onready var dlc_info: Node3D = $DLCInfo
...
@onready var dlc_label: Label3D = $DLCInfo/DLCLabel
...
func update_dlc_info():
	var result: MetaPlatformSDK_Message
	result = await MetaPlatformSDK.asset_file_get_list_async().completed
	if result.is_error():
		dlc_label.text = "Error getting asset file list!"
		push_error("Error asset file list: ", result.error)
		dlc_processing = false
		return

	var asset_array := result.get_asset_details_array()
	var dlc_asset_details: MetaPlatformSDK_AssetDetails = null
	for asset_details: MetaPlatformSDK_AssetDetails in asset_array:
		if asset_details.filepath.contains(DLC_FILENAME):
			dlc_asset_details = asset_details
			break

	if dlc_asset_details == null:
		dlc_label.text = "Error finding DLC asset file!"
		dlc_processing = false
		return

	if dlc_asset_details.iap_status == "not_entitled":
		dlc_label.text = "Not entitled to DLC!"
		dlc_processing = false
		return

	var just_downloaded := false
	if dlc_asset_details.download_status == "available":
		result = await MetaPlatformSDK.asset_file_download_by_id_async(dlc_asset_details.asset_id).completed
		if result.is_error():
			dlc_label.text = "Error downloading DLC!"
			dlc_processing = false
			return
		else:
			just_downloaded = true

	if dlc_asset_details.download_status == "installed" or just_downloaded:
		var file = FileAccess.open(dlc_asset_details.filepath, FileAccess.READ)
		if file == null:
			dlc_label.text = "Error opening DLC file!"
			dlc_processing = false
			return
		dlc_label.text = file.get_as_text()
		file.close()
		dlc_displayed = true
...
func display_dlc():
	if dlc_processing or dlc_displayed:
		return

	dlc_label.text = "Processing..."
	dlc_processing = true

	# This App's example DLC is listed as free in the Meta Quest Developer Dashboard, so purchase is not actually needed.
	# Launching checkout flow for a free product like this will cause a panel to appear momentarily.
	var result: MetaPlatformSDK_Message = await MetaPlatformSDK.iap_launch_checkout_flow_async(DLC_PRODUCT_SKU).completed
	if result.is_error():
		dlc_label.text = "Error launching\nDLC checkout flow!"
		dlc_processing = false;
		return
